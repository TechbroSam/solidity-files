...
contract AutoTrader {
    ...
    constructor(address _uniswap, address _tokenAddress, uint256 _tradeAmount) {
    ...
    // Perform the trade
    uniswap.swapExactTokensForTokens(tradeAmount, amountOutMin, path, address(this), block.timestamp);
    ...
    IERC20(tokenAddress).approve(address(uniswap), tokensToSell);
    uniswap.swapExactTokensForTokens(tokensToSell, 0, path, address(this), block.timestamp);
